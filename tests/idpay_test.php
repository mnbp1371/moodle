...
class enrol_idpay_testcase extends advanced_testcase {
    ...
    public function test_basics() {
        $this->assertFalse(enrol_is_enabled('idpay'));
        $plugin = enrol_get_plugin('idpay');
        $this->assertInstanceOf('enrol_idpay_plugin', $plugin);
        $this->assertEquals(ENROL_EXT_REMOVED_SUSPENDNOROLES, get_config('enrol_idpay', 'expiredaction'));
    }
    ...
    public function test_sync_nothing() {
        ...
        $this->enable_plugin();
        $idpayplugin = enrol_get_plugin('idpay');

        // Just make sure the sync does not throw any errors when nothing to do.
        $idpayplugin->sync(new null_progress_trace());
    }
    ...
    public function test_expired() {
        ...
        /** @var enrol_idpay_plugin $idpayplugin  */
        $idpayplugin = enrol_get_plugin('idpay');
        ...
        $idpayplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $idpayplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $idpayplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $idpayplugin->enrol_user($instance2, $user1->id, $studentrole->id, 0, 0);
        $idpayplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now-60*60);
        $idpayplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now+60*60);

        $idpayplugin->enrol_user($instance3, $user1->id, $teacherrole->id, $now-60*60*24*7, $now-60);
        $idpayplugin->enrol_user($instance3, $user4->id, $teacherrole->id);
        ...
        $idpayplugin->set_config('expiredaction', ENROL_EXT_REMOVED_KEEP);
        $code = $idpayplugin->sync($trace);
        $this->assertSame(0, $code);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        $this->assertEquals(9, $DB->count_records('role_assignments'));


        $idpayplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $idpayplugin->sync($trace);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        ...
        $idpayplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
